...
class TestDeriveKey:
    def test_deterministic(self):
        from neuralbook.encryption import derive_key
        assert derive_key("same-seed-abcdefghij1234567890abcdefghijkl") == derive_key("same-seed-abcdefghij1234567890abcdefghijkl")
...
class TestEncryptDecrypt:
    def test_round_trip(self):
        from neuralbook.encryption import encrypt_content, decrypt_content
        key = _key()
    ...
    def test_blob_format_size(self):
        from neuralbook.encryption import encrypt_content, IV_LENGTH, AUTH_TAG_LENGTH
        key = _key()
        pt = b"payload"
...
class TestEncryptFile:
    def test_file_round_trip(self, tmp_path):
        from neuralbook.encryption import encrypt_file, decrypt_file
        key = _key()
        src = tmp_path / "in.txt"
    ...
    def test_encrypted_size(self, tmp_path):
        from neuralbook.encryption import encrypt_file, IV_LENGTH, AUTH_TAG_LENGTH
        key = _key()
        src = tmp_path / "f.txt"
        enc = tmp_path / "f.nd"
...
class TestKeyDiscovery:
    ...
    def test_config_seed_blocked_for_release(self, monkeypatch):
        from neuralbook.encryption import discover_key, KeySourceError
        monkeypatch.delenv("NEURALBOOK_ENCRYPTION_SEED", raising=False)
