...
import hashlib, json, os, secrets, warnings
...
def discover_key(title_config=None, title_dir=None, build_type="demo"):
    ...
    # 2. Seed file
    seed_file = cfg.get("encryptionSeedFile", "")
    if seed_file:
    ...
    config_seed = cfg.get("encryptionSeed", "").strip()
    if config_seed:
        if build_type in ("release", "external"):
            raise KeySourceError(f"Plaintext config seed not allowed for build_type='{build_type}'")
        warnings.warn("Using plaintext encryptionSeed from config — use env var for release builds.", stacklevel=2)
...
def key_from_config(config_path, build_type="demo"):
...
def _validate_seed(seed, source):
    ...
    for w in ("password", "123456", "qwerty", "admin", "test"):
        if seed.lower().startswith(w):
            raise KeySourceError(f"Seed from {source} matches weak pattern '{w}'. Use a random value.")
